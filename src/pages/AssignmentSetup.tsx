  ...
  const handleGenerateRubric = () => {
    if (!formState.paperUploaded) {
      toast({
        ...
        description: "Please upload the question paper before generating a rubric",
        variant: "destructive"
      });
      return;
    }
  ...
  const handleSaveAndActivate = () => {
    if (!formState.paperUploaded || !formState.rubricUploaded) {
      toast({
        ...
        description: "Please upload both question paper and rubric before activating",
        variant: "destructive"
      });
      return;
    }
    ...
    <div className="space-y-8">
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-4">
          ...
          <div>
            <h1 className="text-3xl font-bold tracking-tight">Assignment Setup</h1>
            <p className="text-muted-foreground">Configure your assignment before activating</p>
          </div>
        </div>
            ...
            <CardContent>
              <div className="space-y-4">
                <div className="flex items-center gap-3">
                  <div className={`rounded-full p-1 ${formState.paperUploaded ? 'bg-green-100 text-green-600' : 'bg-gray-100 text-gray-400'}`}>
                    <Check size={16} />
                  </div>
                  <span className={formState.paperUploaded ? 'text-green-600' : ''}>Upload Question Paper</span>
                </div>
                <div className="flex items-center gap-3">
                  <div className={`rounded-full p-1 ${formState.rubricUploaded ? 'bg-green-100 text-green-600' : 'bg-gray-100 text-gray-400'}`}>
                    <Check size={16} />
                  </div>
                  <span className={formState.rubricUploaded ? 'text-green-600' : ''}>Set Up Rubric</span>
                </div>
                <div className="flex items-center gap-3">
                  <div className={`rounded-full p-1 ${formState.markingSchemeUploaded ? 'bg-green-100 text-green-600' : 'bg-gray-100 text-gray-400'}`}>
                    <Check size={16} />
                  </div>
                  <span className={formState.markingSchemeUploaded ? 'text-green-600' : ''}>Upload Marking Scheme (Optional)</span>
                </div>
              </div>
            </CardContent>
            <CardFooter>
              <Button 
                variant="outline" 
                className="w-full"
                onClick={() => setFormState(prev => ({ ...prev, paperUploaded: false, rubricUploaded: false, markingSchemeUploaded: false }))}
              >
                Reset Progress
              </Button>
            </CardFooter>
            ...
            <CardContent>
              {formState.paperUploaded ? (
                <div className="flex flex-col items-center justify-center p-6 border rounded-lg bg-green-50">
                  ...
                  <p className="text-green-600 font-medium mb-2">Question Paper Uploaded</p>
                  <p className="text-sm text-green-600 mb-6">Your question paper has been successfully uploaded</p>
                  <div className="flex gap-3">
                    <Button variant="outline" size="sm">
                      View Paper
                    </Button>
                    <Button variant="outline" size="sm" onClick={handleUploadPaper}>
                      Replace
                    </Button>
                  </div>
                </div>
                ...
                <div className="flex flex-col items-center justify-center p-12 border border-dashed rounded-lg">
                  ...
                  <p className="text-sm text-center text-muted-foreground mb-6 max-w-md">
                    Upload your question paper. Our AI will automatically extract questions and prepare for grading.
                  </p>
          ...
          <Card>
            ...
            <CardContent>
              {formState.rubricUploaded ? (
                <div className="flex flex-col items-center justify-center p-6 border rounded-lg bg-green-50">
                  <FileText size={60} className="text-green-600 mb-4" />
                  <p className="text-green-600 font-medium mb-2">Rubric Ready</p>
                  <p className="text-sm text-green-600 mb-6">Your rubric has been set up and is ready for grading</p>
                  <div className="flex gap-3">
                    <Button variant="outline" size="sm">
                      View Rubric
                    </Button>
                ...
                <div className="flex flex-col items-center justify-center p-12 border border-dashed rounded-lg">
                  <FileUp size={60} className="text-muted-foreground mb-4" />
                  <p className="text-sm text-center text-muted-foreground mb-6 max-w-md">
                    Either upload your existing rubric or let our AI generate one based on your question paper.
                  </p>
                  ...
                  {!formState.paperUploaded && (
                    <p className="mt-4 text-xs text-amber-600">Upload question paper first to enable AI generation</p>
                  )}
                </div>
              )}
            </CardContent>
          ...
          <Card>
            ...
            <CardContent>
              {formState.markingSchemeUploaded ? (
                <div className="flex flex-col items-center justify-center p-6 border rounded-lg bg-green-50">
                  <FileText size={60} className="text-green-600 mb-4" />
                  <p className="text-green-600 font-medium mb-2">Marking Scheme Uploaded</p>
                  <p className="text-sm text-green-600 mb-6">Your marking scheme has been uploaded successfully</p>
                  <div className="flex gap-3">
                    <Button variant="outline" size="sm">
                      View Marking Scheme
                    </Button>
                    <Button variant="outline" size="sm" onClick={handleUploadMarkingScheme}>
                      Replace
                    </Button>
                  </div>
                </div>
                ...
                <div className="flex flex-col items-center justify-center p-12 border border-dashed rounded-lg">
                  <FileUp size={60} className="text-muted-foreground mb-4" />
                  <p className="text-sm text-center text-muted-foreground mb-6 max-w-md">
                    Upload your marking scheme or solution if available. This will help with the grading process.
                  </p>
